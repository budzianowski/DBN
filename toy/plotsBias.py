...
for itr in range(10):
    ...
    for weight in ww:
        ...
        res = np.vstack((res, [abs(F - default)[0][0], abs(F - sequential)[0][0], abs(F - sequentialTAP)[0][0], abs(F - defaultTAP)[0][0], \
                               ...
                               magMSE(magReal, seqMF), magMSE(magReal, seqTAP)]))
    results[itr] = res
np.save('saves', results)
'''
...
ax1.set_xlabel(r'absolute couplings strength')
plt.show()
fig.savefig('BrandomWeightsRBMZ.pdf')
...
ax2.errorbar(ww+ 0.01, means[:,10], yerr=sd[:, 10], fmt='b^', label=r'sequential MF magn.')
ax2.errorbar(ww+ 0.01, means[:,11], yerr=sd[:, 11], fmt='r^', label=r'sequential TAP magn.')
ax2.legend(loc=2)
#ax2.set_ylim([0, 0.008])
ax2.set_ylabel('MSE')
ax2.set_xlabel(r'absolute couplings strength')
plt.show()
fig.savefig('BrandomWeightsRBMMAG.pdf')
'''
...
ax1.set_xlabel(r'absolute couplings strength')
plt.show()
fig.savefig('BrandomWeightsSignsRBMZ.pdf')
...
ax2.set_xlabel(r'absolute couplings strength')
plt.show()
fig.savefig('BrandomWeightsSignsRBMMAG.pdf')
'''
