...
def energy(lattice, W, n):
    ...
    for ss in range(0, n ** 2):  # over states
        ...
        E -= W[ii][jj][0] * lattice[ii][jj].state * lattice[ii][jj-1].state  # W w * s_i *s_j
        E -= W[ii][jj][1] * lattice[ii][jj].state * lattice[ii][(jj+1) % n].state  # E
        E -= W[ii][jj][2] * lattice[ii][jj].state * lattice[ii-1][jj].state  # N
        E -= W[ii][jj][3] * lattice[ii][jj].state * lattice[(ii+1) % n][jj].state  # S
    E *= 0.5
    return np.exp(-E)
...
def updateMF(lattice, W, n):
    ...
    for state in range(n**2):  # asynchronous update
        ...
        lattice[ii][jj].m = damp * sigmoid(temp) + (1 - damp) * lattice[ii][jj].m
...
def partitionTAP(lattice, W, n):
    ...
    return entropy + naive / 2.0 + tap / 2.0
...
def updateHid(lattice, W, n, method="naive"):
    buf = np.dot(W, lattice.vis)
    if method == "tap":
        buf -= (lattice.hid - 0.5) * (W ** 2).dot(lattice.vis - lattice.vis ** 2)

    return expit(buf)


def updateVis(lattice, W, n, method="naive"):
    buf = np.dot(W.T, lattice.hid)
    if method == "tap":
        buf -= (lattice.vis - 0.5) * (W.T ** 2).dot(lattice.hid - lattice.hid ** 2)
...
def updateRBM3(lattice, W, n, method="naive"): # parallel
    damp = 0.5
    if method == "naive":
        hid = damp * expit(np.dot(W, lattice.vis)) + (1.0 - damp) * lattice.hid
        vis = damp * expit(np.dot(W.T, lattice.hid)) + (1.0 - damp) * lattice.vis
    else:
        hid = damp * expit(np.dot(W, lattice.vis) - (lattice.hid - .5) * np.dot((W ** 2), (lattice.vis - lattice.vis ** 2))) + (1.0 - damp) * lattice.hid
        vis = damp * expit(np.dot(W.T, lattice.hid) - (lattice.vis - .5) * np.dot((W.T ** 2), (lattice.hid - lattice.hid ** 2))) + (1.0 - damp) * lattice.vis
