...
class RBM(object):
    ...
    def get_cost_updates(self, batch, lr=0.01, persistent=None, k=1):
        ...
        for ii in range(k):
            pre_sigmoid_nvs,nv_means,nv_samples,pre_sigmoid_nhs,nh_means,chain = self.gibbs_hvh(chain)
        ...
        self.W += lr * (np.dot(batch.T, ph_sample) - np.dot(chain_end.T, chain))
        #cost = np.mean(self.free_energy(batch)) - np.mean(self.free_energy(chain_end))
        ...
        if self.persistent is None: # todo change it
            ...
            monitoring_cost = self.get_reconstruction_cost(batch, pre_sigmoid_nvs)
