...
class RBM(object):
    ...
    def get_cost_updates(self, batch, lr=0.1, persistent=None, k=1):
        ...
        cost = T.mean(self.free_energy(batch)) - T.mean(
            self.free_energy(chain_end))
        # We must not compute the gradient through the gibbs sampling
        self.W = self.learning_rate * (np.dot( ) - 
	self.vbias = self.learning_rate * ( )
	self.hbias = self.learning_rate * ( )
    ...
    def get_reconstruction_cost(self, batch, pre_sigmoid_nv):
        ...
        cross_entropy = np.mean(
            np.sum(
                batch * np.log(sigmoid(pre_sigmoid_nv)) +
                (1 - batch) * np.log(1 - sigmoid(pre_sigmoid_nv)),
                axis=1
            )
        )
